...
function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(false); // 다크모드 추가
  ...
  const headerBg = isDarkMode ? "bg-slate-900" : "bg-[#f8fafc]";
  ...
  return (
    <div className={`min-h-screen ${bgClass} flex flex-col items-center font-sans overflow-x-hidden relative transition-colors duration-300`}>
      ...
      {/* 헤더 & 네비게이션 */}
      <header className={`w-full max-w-[400px] md:max-w-[700px] sticky top-0 z-40 ${headerBg}`}>
      ...
      <main className="w-full flex flex-col items-center px-6 py-12 pb-32">
        ...
        {activeTab === 'saju' && (
          <div className={cardClass}>
            <div className="text-center py-10">
              ...
              <div className={`p-6 rounded-3xl border border-dashed italic text-[11px] leading-relaxed font-medium ${isDarkMode ? 'bg-slate-700 border-slate-600 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
                "사주팔자는 인생의 설계도와 같습니다."<br/><br/>
                생년월일시 데이터를 바탕으로 AI가<br/>
                당신의 평생 대운과 올해의 횡재수를 정밀 분석합니다.
              </div>
        ...
        {activeTab === 'palm' && (
          <div className={cardClass}>
            <div className="text-center py-10">
              <span className="text-5xl mb-6 block">✋</span>
              <h2 className="text-xl font-black mb-2 italic">손바닥 보물지도</h2>
            ...
            <div className="space-y-4 text-[11px] max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar font-medium">
              {[
                ...
                <div key={i} className={`p-4 rounded-2xl border-l-4 border-yellow-500 shadow-sm ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-50 text-slate-600'}`}>
                  <h3 className={`font-black mb-1 italic ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{i+1}. {item.t}</h3>
                  <p>{item.d}</p>
                </div>
              ))}
              <p className="text-center opacity-30 py-4 italic">... 스크롤하여 더 보기 ...</p>
            </div>
          </div>
        )}
          ...
          <div className={cardClass}>
            ...
            <div className={`overflow-hidden rounded-3xl border shadow-sm ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
              <table className="w-full text-[10px] text-center border-collapse font-medium">
                <thead className={isDarkMode ? 'bg-slate-700 text-slate-400' : 'bg-slate-50 text-slate-400'}>
      ...
      {isPrivacyOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setIsPrivacyOpen(false)}>
          <div className={`${isDarkMode ? 'bg-slate-800 text-slate-200' : 'bg-white text-slate-500'} w-full max-w-[320px] max-h-[70vh] overflow-y-auto rounded-[2rem] p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-300 text-left`} onClick={(e) => e.stopPropagation()}>
            <h3 className={`text-lg font-black mb-6 italic border-b pb-2 tracking-tight ${isDarkMode ? 'text-white border-slate-700' : 'text-slate-900 border-slate-100'}`}>개인정보처리방침</h3>
            <div className="text-[11px] leading-relaxed space-y-5 font-medium">
              <section><p className={`font-bold mb-1 italic ${isDarkMode ? 'text-yellow-400' : 'text-slate-900'}`}>1. 개인정보 수집 미실시</p><p>본 서비스는 사용자의 어떠한 개인정보도 서버에 전송하거나 저장하지 않습니다.</p></section>
              <section><p className={`font-bold mb-1 italic ${isDarkMode ? 'text-yellow-400' : 'text-slate-900'}`}>2. 구글 애드센스 정책</p><p>광고 게재를 위해 구글은 쿠키 정보를 분석에 활용할 수 있습니다.</p></section>
              <section><p className={`font-bold mb-1 italic ${isDarkMode ? 'text-yellow-400' : 'text-slate-900'}`}>3. 서비스 보안 로직</p><p>모든 데이터 처리는 사용자의 기기 로컬 환경에서 즉석 실행됩니다.</p></section>
            </div>
